...
// Define low-precision type as BF16
using floatX = __nv_bfloat16;

// Minimal CUDA error checking macro
#ifndef cudaCheck
...
void attention_forward_cudnn(floatX* out,  // output: (B, T, NH, HS)
    ...
    float attn_scale_cpu = 1.0f / sqrtf((float)HS);
    void* devPtrO = out;
...
#ifndef ENABLE_CUDNN
// Provide a stub so the demo can compile without cuDNN if desired
static void attention_forward_cudnn(floatX*, float*, floatX*, float*, float*, int, int, int, int) {
    fprintf(stderr, "cuDNN support disabled. Compile with -DENABLE_CUDNN and link cudnn_frontend.\n");
}
#endif

// Simple demo: allocate QKV and call attention using thrust::device_vector
int main() {
    // Small toy sizes
    int B = 1;
    int T = 16;      // sequence length
    int NH = 2;      // number of heads
    int HS = 32;     // head size
    int C = NH * HS; // model dim per token
    ...
    size_t elems_qkv = (size_t)B * T * 3 * C; // Q,K,V stacked along 3
    size_t elems_out = (size_t)B * T * C;
    size_t elems_stats = (size_t)B * NH * T; // if needed

    thrust::device_vector<float> h2d_in_fp32(elems_qkv);
    thrust::device_vector<floatX> d_qkv(elems_qkv);
    thrust::device_vector<floatX> d_out(elems_out);
    thrust::device_vector<float> d_out_fp32(elems_out);
    thrust::device_vector<float> d_stats(elems_stats);

    // Initialize inputs with a simple ramp
    thrust::sequence(h2d_in_fp32.begin(), h2d_in_fp32.end(), 0.0f, 1.0f / 1024.0f);

    // Run attention (will internally convert first run and validate)
    attention_forward_cudnn(
        thrust::raw_pointer_cast(d_out.data()),
        thrust::raw_pointer_cast(d_stats.data()),
        thrust::raw_pointer_cast(d_qkv.data()),
        thrust::raw_pointer_cast(h2d_in_fp32.data()),
        thrust::raw_pointer_cast(d_out_fp32.data()),
        B, T, C, NH);

    // Copy a few results back to host and print
    thrust::host_vector<float> h_out_fp32(8);
    thrust::copy(d_out_fp32.begin(), d_out_fp32.begin() + (ptrdiff_t)h_out_fp32.size(), h_out_fp32.begin());
    for (size_t i = 0; i < h_out_fp32.size(); ++i) {
        printf("out[%zu] = %f\n", i, h_out_fp32[i]);
    }

#ifdef ENABLE_CUDNN
    if (cudnn_workspace) cudaCheck(cudaFree(cudnn_workspace));
